...
def load_data():
    try:
        ...
        df = pd.read_csv(os.path.join(BASE_DIR, "../data/cryptoscope_cleaned.csv"))
        return df
    except FileNotFoundError:
        st.error(
            "data/cryptoscope_cleaned.csv not found. Run section1_data_collection.ipynb then section2_analysis_model.ipynb first."
        )
        st.stop()
...
if "fear_greed_score" in df.columns:
    ...
    fg_label = (
        df["fear_greed_label"].iloc[0] if "fear_greed_label" in df.columns else ""
    )
...
if page == "Market Overview":
    ...
    st.caption(
        "Real-time crypto market intelligence. Use the sidebar filters to explore the data."
    )
    st.markdown("---")
    ...
    with col_right:
        tier_signal = (
            df_filtered.groupby(["risk_tier", "market_signal"])
            .size()
    ...
    top20["price_change_pct_24h"] = top20["price_change_pct_24h"].apply(
        lambda x: f"{x:+.2f}%"
    )
    top20.columns = [
        "Name",
    ...
    st.caption(
        "Interactive charts exploring price behavior, volatility, and market structure."
    )
    st.markdown("---")
    ...
    if chart_choice == "Price Change Distribution":
        ...
        st.info(
            f"Mean 24h change: {df_filtered['price_change_pct_24h'].mean():.2f}% | "
        ...
        st.plotly_chart(fig, use_container_width=True)
        st.info(
            "Strong positive correlation confirms larger coins attract more trading activity."
        )
        ...
        with col1:
            ...
            st.plotly_chart(fig, use_container_width=True)
        with col2:
            top_vol = df_filtered[df_filtered["is_stablecoin"] == 0].nlargest(
                15, "volatility_24h_pct"
            )
            fig = px.bar(
                top_vol,
                x="volatility_24h_pct",
                y="name",
                orientation="h",
                title="Top 15 Most Volatile Coins",
                color="volatility_24h_pct",
        ...
        try:
            df_fg = pd.read_csv(os.path.join(BASE_DIR, "../data/fear_greed.csv"))
            ...
            fig = px.line(
                ...
                labels={
                    "fg_date": "Date",
                    "fear_greed_score": "Score (0=Fear, 100=Greed)",
                },
            )
            ...
            st.warning(
                "data/fear_greed.csv not found. Run section1_data_collection.ipynb first."
            )
        ...
        fig = px.box(
            ...
            y="pct_below_ath",
            color="risk_tier",
            title="ATH Recovery by Risk Tier",
            category_orders={"risk_tier": ["Mega", "Large", "Mid", "Small"]},
            labels={"pct_below_ath": "% Below ATH", "risk_tier": "Risk Tier"},
        )
        st.plotly_chart(fig, use_container_width=True)
        st.info(
            "Negative values indicate how far below the all-time high each coin is currently trading."
        )
    ...
    search = st.text_input(
        "Search by coin name or symbol", placeholder="e.g. Bitcoin or BTC"
    )
    ...
    if len(results) > 0:
        selected_coin = st.selectbox(
            "Select a coin to view details", options=results["name"].tolist()
        )
        ...
        with col_a:
            ...
            st.write(
                f"Stablecoin: `{'Yes' if coin_data['is_stablecoin'] == 1 else 'No'}`"
            )
        ...
        fig_gauge = go.Figure(
            go.Indicator(
                mode="gauge+number",
                value=(
                    coin_data["price_to_ath_ratio"] * 100
                    if pd.notna(coin_data["price_to_ath_ratio"])
                    else 0
                ),
    ...
    st.caption(
        "Enter coin market data to predict whether the 7-day return will be positive."
    )
    st.markdown("---")

    st.info(
        "This model uses Random Forest trained on 1000 coins. Input values to get a live prediction."
    )

    col1, col2, col3 = st.columns(3)

    with col1:
        inp_change_24h = st.number_input(
            "24h Price Change (%)", value=0.0, step=0.1, format="%.2f"
        )
        inp_change_30d = st.number_input(
            "30d Price Change (%)", value=0.0, step=1.0, format="%.2f"
        )
        inp_volatility = st.number_input(
            "24h Volatility (%)", value=3.0, min_value=0.0, step=0.5
        )
        inp_volume = st.number_input(
            "24h Volume (USD millions)", value=100.0, min_value=0.1, step=10.0
        )
        inp_mcap = st.number_input(
            "Market Cap (USD millions)", value=1000.0, min_value=0.1, step=100.0
        )
    ...
    with col2:
        ...
        inp_pct_ath = st.number_input(
            "% Below ATH (negative value)", value=-40.0, step=1.0
        )

    with col3:
        inp_momentum = st.number_input(
            "Momentum Score", value=0.0, step=0.5, format="%.2f"
        )
        inp_vol_mcap = st.number_input(
            "Volume/MCap Ratio", value=0.05, min_value=0.0, step=0.01, format="%.4f"
        )
        inp_price_ath = st.number_input(
            "Price/ATH Ratio", value=0.5, min_value=0.0, max_value=1.0, step=0.01
        )
        inp_supply = st.number_input(
            "Supply Utilization (0-1)",
            value=0.7,
            min_value=0.0,
            max_value=1.0,
            step=0.01,
    ...
    if st.button("Run Prediction", type="primary"):
        ...
        user_input = np.array(
            [
                [
                    ...
                    inp_change_30d,
                    inp_volatility,
                    np.log1p(inp_volume * 1e6),
                    np.log1p(inp_mcap * 1e6),
                    inp_fg,
                    inp_fg_avg,
                    inp_shift,
                    inp_news,
        ...
        st.caption(
            "Note: This model is trained on a snapshot of 1000 coins. Crypto markets are highly unpredictable. This is for educational purposes only, not financial advice."
        )
